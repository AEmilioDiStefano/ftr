...
class TemperatureMachine(StateMachine):
   ...
   ##
    ...
    async def updateDB(self):
        ...
        reductstore_url = "http://192.168.8.232:8383/"  
        ...
        temperature = random.uniform(20.0, 30.0)  # In Celsius
        timestamp = int(datetime.timezone.utc().timestamp() * 1e6)  # Microseconds since epoch
        data = str(temperature).encode("utf-8")

        # Write the data
        await bucket.write(current_temperature, data, timestamp)
        print(f"Recorded temperature: {temperature:.2f}°F at {datetime.timezone.utc()}")
